...
my %options=();
getopts("hc:n:m:l:i:d:r:o:", \%options);
...
if (defined $options{o}) {$Outdir = $options{o}} else {$Outdir = "Outputs"};

my $Outname;
if ( -d "$Outdir") {} else { system ("mkdir -p $Outdir")};
if ($ARGV[1]) {$Outname = "$Outdir/$ARGV[1]"} else {$Outname = "$Outdir/Output"};

my $suf = dirname($0);
my $Initial_TAL_strings = $suf . "/Info/Initial_TAL_strings.txt";
my $arlem = $suf. "/arlem";
...
open my $Inital, "<$Initial_TAL_strings";
my @Ini_repeat;
...
$MATfile = $suf . "/Info/XOLS_Repeatmatrix.mat"; #MOD AGAIN FOR AVRXA7, CHANGE BACK!!
$CLASSINfile = $suf . "/Info/XOLS_Repeatscode.txt";
