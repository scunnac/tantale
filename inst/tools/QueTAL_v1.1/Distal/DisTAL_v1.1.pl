...
by considering each repeat as a unit
...
-m :[T or F]: Create new repeat matrix (default = FALSE)
-i: [number]: indel penalization for arlem
-d: [number]: duplication penalization for arlem
...
my $normscore = (($maxlen - $score) / $maxlen) * 100 ; #Normalized alignement score
...
for (my $i=0;$i<@IDs;$i++) {
	...
	if (defined $Scores[$i][$j]) {$val = $Scores[$i][$j] /100; print $OUT5 "$val\t"}
