...
############################################################################
			QueTAL - DisTAL_OnlyRVDs: 

This script takes a fasta file of either nucleotide or aminoacid sequences of
TAL effectors and calculates distances based on sequences of RVDs
...
if (defined $options{m}) {$newMatrix = $options{m}} else {$newMatrix = "T"};
   ...
   #exclude sequences for which no TAL repetition was found 
	...
	for ( my $i = 0 ; $i <= $#Repeat_len ; $i++ ) {
		...
		for  (my $k = 1 ; $k <= $times ; $k++ ) {
						if ($ruse eq "T") {$repeat = substr ($seq, $ini + 11, 2)} 
			...
			if ($Repeat_len[$i] == $common_len - 1) { #repats with N* or H*
				if ($ruse eq "T") {$repeat = substr ($seq, $POS[$i] + 11, 1)}
				push @Repeat_array, $repeat; #print "$repeat\n"
				}

			else {
				if ($ruse eq "T") {$repeat = substr ($seq, $POS[$i] + 11, 2)}
				push @Repeat_array, $repeat; #print "$repeat\n" 
				}
	...
	#get last repeat		
		my $last_repeat;
		if ($ruse eq "T") {$last_repeat = substr ($seq, $POS[$#POS] + 11, 2)}
...
if (defined $OryMat[$i]) {print $OUT3 "$OryMat[$i]$normscore \n"} else {print $OUT3 "$normscore \n"}
		}
...
for (my $i = 0; $i <= $#Uniqrepeats; $i++) {$ClassRep{$Uniqrepeats[$i]} = $i};
...
my @Arlemoutput = `./arlem -f $CodedRepeats -cfile $MATfile -align -insert`;
...
for (my $i=0;$i<@IDs;$i++) {
	...
	if (defined $Scores[$i][$j]) {$val = $Scores[$i][$j] /100; print $OUT5 "$val\t"} #not normalized
	elsif (defined $Scores[$j][$i]) {$val = $Scores[$j][$i] /100; print $OUT5 "$val\t"}
...
pdf("$Outname.pdf")
plot(tr,"$Treetype", tip.color=colvector, show.tip.label=TRUE, cex=0.6)
