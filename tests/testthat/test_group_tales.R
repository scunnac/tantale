...
test_that("tales_group() with k-medoids and an explicit k returns the expected number of groups", {
  grp <- tales_group(tal_sim = tal_sim, k = 3, k_range = 2:6, method = "k-medoids")
...
test_that("tales_group() with k-medoids and k = 'auto' picks a number of groups automatically", {
  expect_message(
    grp <- tales_group(tal_sim = tal_sim, k = "auto", k_range = 2:6, method = "k-medoids"),
...
test_that("tales_group() with k-medoids forces plot_tree off and warns via message", {
  expect_message(
    tales_group(tal_sim = tal_sim, k = 3, k_range = 2:6, method = "k-medoids", plot_tree = TRUE),
...
test_that("tales_group() with k-medoids errors on invalid k_range", {
  expect_error(
    tales_group(tal_sim = tal_sim, k = 3, k_range = "not numeric", method = "k-medoids"),
    "invalid k values"
  )
  expect_error(
    tales_group(tal_sim = tal_sim, k = 3, k_range = NULL, method = "k-medoids"),
    "invalid k values"
  )
})
...
test_that("tales_group() with k-medoids errors on invalid k", {
  ...
  expect_error(
    tales_group(tal_sim = tal_sim, k = c(2, 3), k_range = 2:6, method = "k-medoids")
  )
})

test_that("tales_group() with hclust and an explicit k returns the expected number of groups", {
  grp <- tales_group(tal_sim = tal_sim, k = 3, method = "hclust")
  expect_s3_class(grp, "data.frame")
  expect_named(grp, c("name", "group"))
  expect_equal(nrow(grp), nTales)
  expect_equal(length(unique(grp$group)), 3)
...
test_that("tales_group() with hclust errors when k is not a single number", {
  expect_error(tales_group(tal_sim = tal_sim, method = "hclust"))
  expect_error(tales_group(tal_sim = tal_sim, k = c(2, 3), method = "hclust"))
})
