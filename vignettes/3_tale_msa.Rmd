...
Under the hood, the `build_repeat_msa` function leverages the capacity of [MAFFT](https://mafft.cbrc.jp/alignment/software/) to align arbitrary characters in hexadecimal code to generate TALE parts sequences multiple alignments as illustrated here.
...
rvdVectors <- split_list(telltale_rvd, sep = "-")
repeat_vecs <- split_list(distalr_deci_output$coded.repeats.str, sep = " ")
...
repeat_sim <- distalr_deci_output$repeat.similarity
...
repeatMsaByGroup_withSim <- lapply(sort(unique(taleGroups$group)), function(G) {
  try({build_repeat_msa(input_seqs = repeat_vecs[taleGroups$name[taleGroups$group == G]],
                      sep = " ",
...
For this type of plot, we need to map repeatID to RVD with `repeat_to_rvd_map()`, then convert repeat MSA to RVD MSA by `repeat_to_rvd_align`. We should use the converting function rather than using `build_repeat_msa()` to creat MSA of RVD because there will be disagreement between them.
...
# plot MSA
tantale::msa_heatmap(tal_sim = distalr_deci_output$tal.similarity,
