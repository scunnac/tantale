...
```{r outdir}
outdir <- "/tmp/test_tantale"
dir.create(outdir, showWarnings = F)
telltale2_outdir <- file.path(outdir, "telltale2")
dir.create(telltale2_outdir, showWarnings = F)
```
...
```{r telltale_no_correction, eval=FALSE}
mclapply(c(mai1_fa, bai3_fa), function(fastaFile) {
  ...
  tellTale2(subjectFile = fastaFile,
            outputDir = outputDir,
...
```{r telltale2_correction, eval=FALSE}
correction_reference <- system.file("extdata", "AA_ref.fa.gz", package = "tantale", mustWork = T)

tellTale2(subjectFile = bai311_fa,
          outputDir = file.path(telltale2_outdir, gsub(".fa", "_correction", basename(bai311_fa))),
          appendExtremityCodes = TRUE,
          rvdSep = "-",
          talArrayCorrection = TRUE,
          refForTalArrayCorrection = correction_reference,
...
for (f in arrayReport_files) {
  ...
  if (!any(c("predicted_ins_count", "predicted_dels_count") %in% colnames(a))) a[c("predicted_ins_count", "predicted_dels_count")] <- NA # 2 columns "predicted_ins_count" and "predicted_dels_count" are not displayed in the report in case of no correction
...
`predicted_ins_count`  | number of insertions have been corrected by `telltale2()`
`predicted_dels_count` | number of deletions have been corrected by `telltale2()`
...
distal_telltale_input <- list.files(telltale2_outdir, "putativeTalOrf.fasta", recursive = T, full.names = T)
distal_telltale_input <- distal_telltale_input[!grepl("temp_annotale", distal_telltale_input, fixed = T)]
...
Based on the silhoutte values, `groupTales()` will take the value at the elbow of the curve. 
In this case it picked k = 9, and it makes sense as we already know that the each of the gold genomes have 9 Tals.


### msa with similarity
We can make MSA of repeatID sequence or RVD sequence with the function `buildRepeatMsa()`, however it make more sense to align repeat than RVD.
For mafft, the combo of gap opening score = 0 and gap extending score = 5 creates better alignment than default values.
...
slctPromoters <- c("SWEET14p_Kitaake_or_Nip_?_Sense", "SWEET13p_Nipponbare_Sense", "SWEET11p_Nipponbare_Sense")
...
grFilter <- "SWEET14p_Kitaake_or_Nip_?_Sense:340-370"
...
grFilter <- "SWEET14p_Kitaake_or_Nip_?_Sense:420-450"
plotTaleTargetPred(predResults = selectedPreds, subjDnaSeqFile = subjDnaSeqFile, filterRange = grFilter)
```
