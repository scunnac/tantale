...
rmarkdown::render(input = "/home/cunnac/Lab-Related/MyScripts/tantale/vignettes/workflow.Rmd")
```
...
Genomes used for this vignette are MAI1, BAI3, BAI3-1-1 (an error-prone genome with TalC deleted) and PXO86, a outgroup Asian strain.\
...
`telltale()` adds NTERM and CTERM to RVD sequences as markers of the termini, that is important to map the repeat codes by [DisTal](https://doi.org/10.3389/fpls.2015.00545) with the RVDs.
...
We run DisTal for all the putative TALE ORFs (from `telltale2` output).
...
```{r correlation of TALE distances from various distal implementations, fig.width = 4, fig.height=4}
...
# pairs(talSimOutAgregated[3:5], pch = 19, lower.panel = NULL)
...
# A few words on the various objects in `tantale`


# TALE domains multiple alignments with similarity

If interested in performing TALEs multiple alignments (distal domains or RVDs) it makes sense to align TALEs that are related and that thus belong to the same group. This is what we are doing here but the `buildRepeatMsa()` function can take any arbitrary set of repeatID sequences or RVD sequences.
