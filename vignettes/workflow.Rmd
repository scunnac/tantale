...
```{r correlation of TALE distances from various distal implementations}
library('corrr')
library(ggcorrplot)
...
getTALEsTree <- function(taleSim) {
  ...
  hclust(as.dist(distMat), method = "ward.D")
}
...
distal_output$tal.similarity$TAL1 %>% unique()


distal_output$tal.similarity

reshape2::acast(dissimLong, formula = subj ~ pattern, value.var = "Dissim")

distalALV <- hclust(as.dist(distal_output$tal.similarity), method = "ward.D")
```
...
repeatMsaByGroup_withSim <- lapply(sort(unique(taleGroups$group)), function(G) {
  ...
  repeatMsa <- buildRepeatMsa(inputSeqs = repeatVectorsForGroup,
                              ...
                              mafftOpts = "--globalpair --weighti 1 --maxiterate 1000 --reorder --op 0 --ep 5 --thread 8")
  }
)
```
...
# plot MSA
tantale::heatmap_msa(talsim = distal_output$tal.similarity,
