...
partsForPlots %>% filter(arrayID == "BAI3-1-1_ROI_00007")

partsForPlots %>% filter(is.na(aaSeq))

partsForPlots %>% ggplot(mapping = aes(fill = domainType, y = arrayID, x = position), color = isNaAaSeq) +
  geom_point(shape = 21)


distalr_bs_output <- distalr(taleParts = partsForPlots %>% filter(grepl("MAI1", arrayID)),
                             repeats.cluster.h.cut = 10, ncores = 6,
                             pairwiseAlnMethod = "Biostrings")

distalr_mms_output <- distalr(taleParts = partsForPlots %>% filter(grepl("MAI1", arrayID)),
                              repeats.cluster.h.cut = 10, ncores = 6,
