...
talSimOutAgregatedLong %>%
  dplyr::filter(distalALV != 1 | distalBIOS != 1 | distalDECI != 1) %>%
  arrange(TAL1, TAL2) %>%
  print(n = 200)
...
domSimOutAgregated <- list(distalALV = distal_output$repeat.similarity,
     distalDECI = distalr_deci_output$repeat.similarity,
     distalBIOS = distalr_bs_output$repeat.similarity
) %>% bind_rows(.id = "method")

domSimOutAgregatedLong <- reshape2::dcast(domSimOutAgregated %>% select(method, RepU1, RepU2, Sim),
                                          formula = RepU1 + RepU2 ~ method, value.var = "Sim"
                                            ) %>% as_tibble()

domSimOutAgregatedLong %>%
  dplyr::filter(is.na(distalALV) | is.na(distalBIOS) | is.na(distalDECI)) %>%
  arrange(RepU1, RepU2) %>%
  print(n = 200)
