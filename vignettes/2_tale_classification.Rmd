...
To classify TALE groups, we use the function `distalr()`, an R implementation of the original DisTal Perl program (see below), which outputs similarity between TALEs and similarity between repeats. The TALE similarity table can be taken as input of `group_tales()` for clustering groups, and the repeats similarity can be used for plotting multiple alignment of Tals within group.

> `distalr()` superseded the original Perl-based `runDistal()`/`buildDisTalGroups()` functions, which have been removed from the package (along with the ~90MB of bundled Perl modules they required). Before removal, their output was benchmarked against `distalr()`'s three pairwise-alignment backends and found to correlate highly (see the archived comparison in an earlier version of this vignette, or git history for `R/distal.R`).
...
tale_parts <- lapply(list.dirs(telltale_outdir, recursive = FALSE), function(ttGenomeDir) {
  ...
  tale_parts <- tale_parts(ttGenomeDir) %>%
...
system.time({distalr_bs_output <- distalr(tale_parts = partsForDistalr,
                             h_cut = 10, ncores = 6,
                             aln_method = "Biostrings")})

system.time({distalr_deci_output <- distalr(tale_parts = partsForDistalr,
                              h_cut = 10, ncores = 6,
                              aln_method = "DECIPHER")})

system.time({distalr_mms_output <- distalr(tale_parts = partsForDistalr,
                              h_cut = 10, ncores = 6,
                              aln_method = "mmseq2",
...
talSimOutAgregated <- list(distalDECI = distalr_deci_output$tal.similarity,
...
The correlation of the TALE pairwise similarity scores output by the various `distalr()` backends is remarkably high.

## Allocate TALEs to groups of related sequences

```{r tale trees, fig.width = 6, fig.height=6}
k <- 22
groupsDECI <- invisible(group_tales(tal_sim = distalr_deci_output$tal.similarity,
                     plot_tree = TRUE, k = k, k_range = 10:40,
                     method = "k-medoids"))

groupsBIOS <- invisible(group_tales(tal_sim = distalr_bs_output$tal.similarity,
                     plot_tree = TRUE, k = k, k_range = 10:40,
                     method = "k-medoids"))

groupsMMS <- invisible(group_tales(tal_sim = distalr_mms_output$tal.similarity,
                     plot_tree = TRUE, k = k, k_range = 10:40,
                     method = "k-medoids"))


grp <- group_tales(tal_sim = distalr_deci_output$tal.similarity,
                     plot_tree = TRUE, k = k,
                     method = "hclust")
grp <- group_tales(tal_sim = distalr_bs_output$tal.similarity,
                     plot_tree = TRUE, k = k,
                     method = "hclust")
grp <- group_tales(tal_sim = distalr_mms_output$tal.similarity,
                     plot_tree = TRUE, k = k,
                     method = "hclust")
...
Based on the silhouette values, `group_tales()` will take the value at the elbow of the curve. In this case we manually took k = `r k`.
