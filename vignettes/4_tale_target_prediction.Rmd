...
vignette: >
  %\VignetteIndexEntry{4. TALE targets predictions with tantale}
...
subj_file <-  system.file("extdata", "cladeIII_sweet_promoters.fasta", package = "tantale", mustWork = T)
readBStringSet(subj_file) %>% names
```
...
```{r talvez, eval = TRUE}
talvezPreds <- talvez(rvd_seqs = rvdSeqsXstrings,
                      subj_file = subj_file,
                      opt_param = "-t 0 -l 19",
...
To see how RVD sequences match a promoter region, we supply the prediction table and the promoter sequence with the genomic range to `plot_target_preds()`:

```{r SWEET13, fig.width = 14, fig.height = 4}
grFilter <- "SWEET14p_Nipponbare_Sense:340-450"
plot_target_preds(preds = selectedPreds, subj_file = subj_file, filter_range = grFilter)
```
...
preditalePreds <- preditale(rvd_seqs = rvdSeqsXstrings, subj_file = subj_file, output_dir = NULL)
