...
This demo will illustrate how to use the `build_repeat_msa` function to produce TALE sequence multiple algnments.\
...
```{r align rvd strings}
aln <- build_repeat_msa(input_seqs = system.file("extdata", "TalA_RVDSeqs_AnnoTALE.fasta",
...
Now, it is more evolutionarily relevant to look at Distal parts/domain/repeat (this is used somehow interchangeably with roughly the same meaning) code sequences. Needless to say that to obtain TALE parts code sequences, you need beforehand to run `distalr()` with DNA sequences. In this cas we just use a toy example provided with tantale.


```{r align parts strings}
aln <- build_repeat_msa(input_seqs = system.file("extdata", "small_Out_CodedRepeats.fa",
                                              package = "tantale", mustWork = TRUE),
                      sep = " ",
                      repeat_sims = NULL,
                      mafft_opts = "--localpair --maxiterate 1000 --quiet --reorder --op 0 --ep 5 --thread 1",
