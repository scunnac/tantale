...
mclapply(c(mai1_fa, bai3_fa, pxo86), function(fastaFile) {
  ...
  output_dir <- file.path(telltale_outdir, outdirname)
  tell_tales(subject_file = fastaFile,
...
tell_tales(subject_file = bai311_fa,
         output_dir = file.path(telltale_outdir, gsub(".fa", "_correction", basename(bai311_fa))),
         cterm_min_score = 300,
         extremity_codes = TRUE,
         rvd_sep = "-",
         correct_array = TRUE)
...
arrayRp_correction <- reshape2::melt(arrayRp[c("strain", "array_id", "predicted_ins_count", "predicted_dels_count")], value.name = "count", variable.name = "type")
arrayRp_correction %<>% dplyr::arrange(strain, array_id)
...
ggplot(na.omit(arrayRp_correction), aes(x = array_id, y = count, color = type, shape = type)) +
